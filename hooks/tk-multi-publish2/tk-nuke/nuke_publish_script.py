...
class NukeSessionPublishPlugin(HookBaseClass):
    ...
    setting for this plugin should look something like this::
        ...
        hook: "{self}/publish_file.py:{config}/tk-multi-publish2/tk-nuke/nuke_publish_script.py"
    ...
    def validate(self, settings, item):
        ...
        if next_version_path and os.path.exists(next_version_path):
            ...
            warning_msg = "The next version of this file already exists on disk so this file will not automatically version up unless you save to next available version."
            self.logger.warning(
                warning_msg,
                extra={
                    "action_button": {
                        ...
                        "v%s" % (version,),
                        "callback": lambda: _save_and_allow_version_up(next_version_path, item),
                    }
                },
            )
    ...
    def finalize(self, settings, item):
        ...
        if item.properties.get("version_up"):
...
def _save_and_allow_version_up(path, item):
    """
