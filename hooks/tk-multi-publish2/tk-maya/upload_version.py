...
class UploadVersionPlugin(HookBaseClass):
    ...
    def settings(self):
        ...
        return {
            ...
            "Playblast Publish Template": {
                "type": "template",
                "default": None,
                "description": "Template path for published playblasts. Should"
            ...
            "Playblast Work Template": {
                "type": "template",
                "default": None,
                "description": "Template path for working playblasts. Should"
                "correspond to a template defined in "
                "templates.yml.",
            },
    ...
    def publish(self, settings, item):
        ...
        # get the fields from the work template
        work_template = item.properties["work_template"]
        fields = work_template.get_fields(path)

        # get the publish template location
        publish_template = item.properties["publish_template"]
        publish_path = publish_template.apply_fields(fields)

        # move the playblast to the publish location
        shutil.move(path, publish_path)
        ...
        version_data["sg_first_frame"] = start
        version_data["sg_last_frame"] = end
        version_data["frame_count"] = int(end - start + 1)
        version_data["frame_range"] = "{}-{}".format(start, end)
