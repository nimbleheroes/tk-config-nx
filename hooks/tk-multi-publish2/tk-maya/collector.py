...
SSET_PATTERN = r"^pub_(?P<pubType>[a-zA-Z0-9]+)(_(?P<token>[a-zA-Z0-9]+))?$"
...
class MayaSessionCollector(HookBaseClass):
    ...
    def process_current_session(self, settings, parent_item):
        ...
        all_ssets = cmds.ls(sets=True, noIntermediate=True)
        r = re.compile(SSET_PATTERN)

        valid_ssets = list(filter(r.match, all_ssets))
        for sset in valid_ssets:
            m = r.search(sset)
            if m.group('pubType') == 'model':
                token = m.group('token')
                self._collect_sset_model(item, sset, token)
    ...
    def _collect_sset_model(self, parent_item, sset, token=None):
        """
        Creates items for sset model to be exported.

        :param parent_item: Parent Item instance
        """

        model_item = parent_item.create_item(
            "maya.sset.model", "Model from Selection Set", sset
        )

        # get the icon path to display for this item
        icon_path = os.path.join(self.disk_location, "..", "icons", "geometry.png")

        model_item.set_icon_from_path(icon_path)
        model_item.properties["sset"] = sset
        model_item.properties["var"] = token
    ...
    #         version = publisher.util.get_version_number(movie_path)

    #         # the item has been created. update the display name to include
    #         # the an indication of what it is and why it was collected
    #         item.name = "%s (%s v%s)" % (item.name, "playblast", version)
